...
#Run crqa (works)
#When this work - loop through all the files (almost done)

#Loading packages:
library(pacman)
p_load(ggplot2,dplyr,groupdata2,crqa,gridExtra)

setwd("C:/Users/Ejer/Desktop/3. semester/Experimental Methods 3/Assignment2")

folder = "C:/Users/Ejer/Desktop/3. semester/Experimental Methods 3/Assignment2/Heart Rate/"
...
#threshold=2.5
#pair1_rescaled$HR1=removeOuts(pair1_rescaled$HR1,threshold)
#pair1_rescaled$HR2 = removeOuts(pair1_rescaled$HR2,threshold)

#pair1_rescaled$Resp1 = removeOuts(pair1_rescaled$Resp1,threshold)
#pair1_rescaled$Resp2 = removeOuts(pair1_rescaled$Resp2,threshold)

#For testing RemoveOutlier function
#df = pair1_rescaled
#removeOutlierl = NULL
...
n = 1 #HUSK DEN HER LINJE FORHELVEDE!
...
#Run craq for one pair, one condition--> remember to find parameters
...
#Defining items to test loops and functions
#dataset = pair1_rescaled
#t1 = "HR1"
#t2 = "HR2"
#n = NA
...
rqa_extractor = function(dataset, t1, t2, embed, delay, radius){
  result = NULL
  t1 = dplyr::select_(dataset, t1)
  t2 = dplyr::select_(dataset, t2)
  result = try(crqa(t1, t2, embed = embed, delay = delay, radius = radius+1.2, normalize = 0, rescale = 0, mindiagline = 2, minvertline = 2))
  if (length(result) > 1) {
...
for (file in filelist){
  ...
  downSampling_df = DownSampling(temp)
  downSampling_df = subset(downSampling_df, select = -c(.groups))
  scaled_df = ReScale(downSampling_df)
  Preprocessed_df = ReMoveOutliers(downSampling_df, threshold = 2.5)
...
#Defining stuff to test the function
dataset = Preprocessed_df
t1 = "HR1"
t2 = "HR2"
embed = opt_param_df$emddim_HR
delay = opt_param_df$delay_HR
radius = opt_param_df$radius_HR
...
test_crqa = rqa_extractor(Preprocessed_df, HR1, HR2, embed = opt_param_df$emddim_HR, delay = opt_param_df$delay_HR, radius = opt_param_df$radius_HR)

temp = Preprocessed_df

for (file in filelist){
  temp = read.csv(file, sep = ",")
  crqa_hr = rqa_extractor(temp, "HR1", "HR2", embed = opt_param_df$emddim_HR, delay = opt_param_df$delay_HR, radius = opt_param_df$radius_HR)
  crqa_resp = rqa_extractor(temp, "Resp1", "Resp2", embed = opt_param_df$emddim_resp, delay = opt_param_df$delay_resp, radius = opt_param_df$radius_resp)
